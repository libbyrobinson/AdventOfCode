    ...
    public class Day2_CorruptionChecksum
    {
        ...
        public int CalculateChecksum_Part2()
        {
            ...
            foreach (var row in rows)
            {
                ...
                {
                    var evenDivisors = numbers.Where(n => n != number && number % n == 0).ToList();
                    if (evenDivisors.Any())
                    {
                        numbersToAdd.Add(number / evenDivisors.First());
                        break;
                    }
                }
            }
