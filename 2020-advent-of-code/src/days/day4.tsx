...
type PassportData = Partial<Record<PassportField, string>>;

function Day4() {
  const batchPassports = [
  ...
  const hasAllRequiredFields = (passportData: PassportData) => {
    let isValid = true;
    Object.values(PassportField).forEach((field: PassportField) => {
      if (!passportData[field] && field !== PassportField.cid) {
        isValid = false;
      }
    });
    return isValid;
  };
  const validYear = (min: number, max: number, value?: string): boolean => {
    if (!validNumber(4, value)) {
      return false;
    }
    const year = Number(value);
    return year >= min && year <= max;
  };
  const validHeight = (value?: string): boolean => {
    if (!value) {
      return false;
    }
    const height = Number(value.substring(0, value.length - 2));
  ...
  const validHairColour = (value?: string): boolean => {
  ...
  const hasAllValidDataFields = (passportData: PassportData) => {
    return (
      validYear(1920, 2002, passportData.byr) &&
      validYear(2010, 2020, passportData.iyr) &&
      validYear(2020, 2030, passportData.eyr) &&
      validHeight(passportData.hgt) &&
      validHairColour(passportData.hcl) &&
      validEyeColour(passportData.ecl) &&
      validNumber(9, passportData.pid)
    );
  };

  const toPassportData = (lines: string): PassportData => {
    const passportData: PassportData = {};
    const keyValues = lines.split(" ");
    keyValues.forEach((keyValueString: string) => {
      const keyValue = keyValueString.split(":");
      if (keyValue[0] in PassportField) {
        passportData[keyValue[0] as PassportField] = keyValue[1];
      }
    });
    return passportData;
  };
  ...
  let currentPassportLines = "";
  batchPassports.forEach((line: string) => {
    if (line) {
      currentPassportLines += `${line} `;
    } else {
      passports.push(toPassportData(currentPassportLines));
      currentPassportLines = "";
    }
  });
  passports.push(toPassportData(currentPassportLines));

  const validPassportsPart1 = passports.reduce(
    (validCount: number, passportData: PassportData) => {
      if (hasAllRequiredFields(passportData)) {
        validCount++;
      }
      return validCount;
    },
  ...
  const validPassportsPart2 = passports.reduce(
    (validCount: number, passportData: PassportData) => {
      if (hasAllValidDataFields(passportData)) {
        validCount++;
      }
      return validCount;
    },
  ...
  return (
    <div>
      <h2>Day 4</h2>
      <p>There are {validPassportsPart1} valid passports in part 1</p>
      <p>There are {validPassportsPart2} valid passports in part 2</p>
    </div>
  );
}
