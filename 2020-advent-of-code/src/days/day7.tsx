...
function Day7() {
  const canBeCarriedBy: Record<string, string[]> = {};
  const mustCarry: Record<string, string[]> = {};

  input.forEach((rule: string) => {
    const insideAndOutsideBags = rule.split(" bags contain ");
    const outsideBagColour = insideAndOutsideBags[0];
    if (!mustCarry[outsideBagColour]) {
      mustCarry[outsideBagColour] = [];
    }

    const insideBags = insideAndOutsideBags[1].split(
      / bags, | bag, | bags.| bag./
    );
    insideBags.forEach((insideBag: string) => {
      const insideBagCount = Number(insideBag[0]);
      const insideBagColour = insideBag.substring(2);
      if (insideBagColour && insideBagColour !== " other") {
        if (!canBeCarriedBy[insideBagColour]) {
          canBeCarriedBy[insideBagColour] = [];
        }
        mustCarry[outsideBagColour].push(
          ...Array(insideBagCount).fill(insideBagColour)
        );
        canBeCarriedBy[insideBagColour].push(outsideBagColour);
      }
    });
  });

  const possibleOutsideBags = (insideColour: string): Set<string> => {
    const outsideBags = new Set<string>();

    const directParents = canBeCarriedBy[insideColour];
    if (directParents) {
      directParents.forEach((directParent: string) => {
        outsideBags.add(directParent);

        const indirectParents = possibleOutsideBags(directParent);
        indirectParents.forEach((indirectParent: string) =>
          outsideBags.add(indirectParent)
        );
      });
    }
  ...
  const allInsideBags = (outsideColour: string): string[] => {
    const insideBags: string[] = [];

    const directlyInside = mustCarry[outsideColour];
    if (directlyInside) {
      directlyInside.forEach((directInside: string) => {
        insideBags.push(directInside);

        const indirectlyInside = allInsideBags(directInside);
        indirectlyInside.forEach((indirectInside: string) =>
          insideBags.push(indirectInside)
        );
      });
    }
  ...
  const part1 = possibleOutsideBags("shiny gold");
  const part2 = allInsideBags("shiny gold");

  return (
    <div>
      <h2>Day 7</h2>
      <p>
        There are {part1.size} bag colours in part 1 that can eventually contain
        your shiny gold bag
      </p>
      <p>Your shiny gold bag in part 2 must contain {part2.length} bags</p>
    </div>
  );
}
