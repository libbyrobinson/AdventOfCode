...
function Day14() {
  const dec2bin = (dec: number, length: number): string => {
    return (dec >>> 0).toString(2).padStart(length, "0");
  };

  const runValueThroughMask = (
    instruction: { address: number; value: number },
    mask: string
  ) => {
    let bin = dec2bin(instruction.value, 36).split("");
    for (var bit = 35; bit >= 0; bit--) {
  ...
  const decodeMemoryAddresses = (
    ...
    let bin = dec2bin(instruction.address, 36).split("");
    ...
    for (var bit = 35; bit >= 0; bit--) {
      if (mask[bit] === "1") {
        ...
        bin[bit] = "X";
        floatingBitLocations.push(bit);
      }
    }

    const xCount = floatingBitLocations.length;

    let addresses: number[] = [];
    for (let i = parseInt("1".repeat(xCount), 2); i >= 0; i--) {
      const replace = dec2bin(i, xCount);
      const newBin = [...bin];
      floatingBitLocations.forEach((location: number, index: number) => {
        newBin[location] = replace[index];
      });
      addresses.push(parseInt(newBin.join(""), 2));
    }
    return addresses;
  };

  const parseMask = (line: string) => {
    return line.split(" = ")[1];
  };

  const parseInstruction = (line: string) => {
    const locationAndValue = line.split(" = ");
    return {
      address: Number(locationAndValue[0].split("[")[1].slice(0, -1)),
      value: Number(locationAndValue[1]),
    };
  };

  const part1 = (input: string[]) => {
    let memory: { [address: number]: number } = {};
    let mask = "";
    ...
    input.forEach((line: string) => {
      if (line.startsWith("mask")) {
        ...
        const newValue = runValueThroughMask(instruction, mask);
        memory[instruction.address] = newValue;
      }
    });

    return Object.values(memory).reduce((sum: number, value: number) => {
      return (sum += value);
    }, 0);
  };
  ...
  const part2 = (input: string[]) => {
    ...
    input.forEach((line: string) => {
      if (line.startsWith("mask")) {
        ...
        const instruction = parseInstruction(line);
        const addresses = decodeMemoryAddresses(instruction, mask);
  ...
  assert(part1(exampleInput), 165);
  assert(part2(exampleInput2), 208);

  return (
    <div>
      <h2>Day 14</h2>
      <p>The sum of all values left in memory for part 1 is {part1(input)}</p>
      <p>The sum of all values left in memory for part 1 is {part2(input)}</p>
    </div>
  );
}
