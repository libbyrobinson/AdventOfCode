...
function Day2() {
  ...
  const passwordsAndRules: PasswordData[] = inputs.map((input: string) => {
    const ruleAndPassword = input.split(":");
    const minMaxAndChar = ruleAndPassword[0].split(" ");
    const minMax = minMaxAndChar[0].split("-");

    const left = Number(minMax[0]);
    const right = Number(minMax[1]);
    const char = minMaxAndChar[1];
    const password = ruleAndPassword[1].trim();
  ...
  const validPasswordPart1 = passwordsAndRules.reduce(
    (validCount: number, passwordData: PasswordData) => {
      const characterCount = (
        passwordData.password.match(new RegExp(passwordData.char, "g")) || []
      ).length;
      if (
        characterCount >= passwordData.left &&
        characterCount <= passwordData.right
      ) {
        validCount++;
      }
      return validCount;
  ...
  const validPasswordPart2 = passwordsAndRules.reduce(
    (validCount: number, passwordData: PasswordData) => {
      const charInPosition = (position: number) => {
        const index = position - 1;
        return passwordData.password[index] === passwordData.char;
      };
      const charInLeftPosition = charInPosition(passwordData.left);
      const charInRightPosition = charInPosition(passwordData.right);
      if (
        (charInLeftPosition || charInRightPosition) &&
        !(charInLeftPosition && charInRightPosition)
      ) {
        validCount++;
      }
  ...
  return (
    <div>
      <h2>Day 2</h2>
      <p>There are {validPasswordPart1} valid passwords in part 1</p>
      <p>There are {validPasswordPart2} valid passwords in part 2</p>
    </div>
  );
}
