...
const regexMatchForIndex = (rules: string[], index: number): string => {
  if (rules[index].includes("a")) {
    return "a";
  }
  if (rules[index].includes("b")) {
    return "b";
  }

  if (rules[index].includes("|")) {
    const [left, right] = rules[index].split(" | ");
    const leftMatch = regexMatch(rules, left);
    const rightMatch = regexMatch(rules, right);
    return `(${leftMatch}|${rightMatch})`;
  } else {
    return regexMatch(rules, rules[index]);
  }
};

const regexMatch = (rules: string[], rule: string) =>
  rule
    .split(" ")
    .reduce(
      (regexMatch: string, ruleIndex: string) =>
        regexMatch + regexMatchForIndex(rules, Number(ruleIndex)),
      "("
    ) + ")";

const parseRules = (ruleInput: string[]): string[] => {
  const rules: string[] = [];
  for (const line of ruleInput) {
    const indexAndRule = line.split(": ");
    const index = Number(indexAndRule[0]);
    rules[index] = indexAndRule[1];
  }
  return rules;
};
...
function Day19() {
  ...
  // part 1
  const part1Regex = RegExp("^" + regexMatchForIndex(rules, 0) + "$");
  const part1 = messageInput.reduce((matchCount: number, message: string) => {
    if (part1Regex.test(message)) {
      matchCount++;
    }
    return matchCount;
  }, 0);
  ...
  //   rule8 += `${new Array(i).fill(42).join(" ")} | `;
  //   rule11 += `${new Array(i)
  //     .fill(42)
  //     .concat(new Array(i).fill(31))
  //     .join(" ")} | `;
  // }
  // rules[8] = rule8.slice(0, rule8.length - 3);
  // rules[11] = rule11.slice(0, rule11.length - 3);

  // const part2Regex = RegExp("^" + regexMatchForIndex(rules, 0) + "$");
  // const part2 = messageInput.reduce((matchCount: number, message: string) => {
  //   if (part2Regex.test(message)) {
  //     matchCount++;
  //   }
  //   return matchCount;
  // }, 0); // should be 363
  ...
  // part 2
  const rule31RegexMatch = regexMatchForIndex(rules, 31);
  const rule42RegexMatch = regexMatchForIndex(rules, 42);
  const rule42n31 = new RegExp(
    "^(?<r42>(" + rule42RegexMatch + ")+)(?<r31>(" + rule31RegexMatch + ")+)$"
  );
  const rule31 = new RegExp(rule31RegexMatch, "g");
  const rule42 = new RegExp(rule42RegexMatch, "g");

  let part2 = 0;
  for (const m of messageInput) {
  ...
  return (
    <div>
      <h2>Day 19</h2>
      <p>There are {part1} matched messages in part 1</p>
      <p>There are {part2} matched messages in part 2</p>
    </div>
  );
}
