...
function Day18() {
  const evaluateWithoutBrackets = (
    expression: string,
    additionFirst: boolean
  ): number => {
    const operationOrNumber = expression
      .split(" ")
      .filter((value) => value !== "");
  ...
  const evaluateWithBrackets = (
    ...
    return evaluateWithoutBrackets(expression, additionFirst);
  };

  // part 1 examples
  assert(evaluateWithBrackets("2 * 3 + (4 * 5)"), 26);
  assert(evaluateWithBrackets("5 + (8 * 3 + 9 + 3 * 4 * 3)"), 437);
  assert(
    evaluateWithBrackets("5 * 9 * (7 * 3 * 3 + 9 * 3 + (8 + 6 * 4))"),
    12240
  );
  assert(
    evaluateWithBrackets("((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2"),
    13632
  );

  // part 2 examples
  assert(evaluateWithBrackets("1 + (2 * 3) + (4 * (5 + 6))", true), 51);
  assert(evaluateWithBrackets("2 * 3 + (4 * 5)", true), 46);
  assert(evaluateWithBrackets("5 + (8 * 3 + 9 + 3 * 4 * 3)", true), 1445);
  assert(
    evaluateWithBrackets("5 * 9 * (7 * 3 * 3 + 9 * 3 + (8 + 6 * 4))", true),
    669060
  );
  assert(
    evaluateWithBrackets(
  ...
  const part1 = input
    .map((line) => evaluateWithBrackets(line))
    .reduce((sum, value) => (sum += value), 0);

  const part2 = input
    .map((line) => evaluateWithBrackets(line, true))
    .reduce((sum, value) => (sum += value), 0);

  return (
    <div>
      <h2>Day 18</h2>
      <p>The sum of all values in part 1 is {part1}</p>
      <p>The sum of all values in part 2 is {part2}</p>
    </div>
  );
}
